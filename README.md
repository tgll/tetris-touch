# Courses projects

### HCID

Vases Voices
------------

**Gestural and Mobile Interactions group project (1st Year of Master degree in Universite Paris-Saclay)**

Touch-based Tetris variant using the 1$ motion recognizer.
Repository link we delivered for this course on [François's github](https://github.com/frcsdes/tetris-touch)


------

### More courses

See the summary of others courses projects [here](https://github.com/tgll/COURSES-PROJECTS-list)
